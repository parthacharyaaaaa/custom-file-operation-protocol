...
from server.info_ops.info_subhandlers import handle_heartbeat, handle_permission_query, handle_filedata_query, handle_user_query
...
INFO_SUBHANDLER_MAPPING: MappingProxyType[int, INFO_SUBHANDLER] = MappingProxyType(
    dict(
        zip(
            [InfoFlags.HEARTBEAT, InfoFlags.PERMISSION_METADATA, InfoFlags.FILE_METADATA, InfoFlags.USER_METADATA],
            [handle_heartbeat, handle_permission_query, handle_filedata_query, handle_user_query]
        )
    )
)
