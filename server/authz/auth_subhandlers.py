...
async def handle_registration(header_component: BaseHeaderComponent,
    ...
    assert auth_component.password

    await user_manager.create_user(username=auth_component.identity, password=auth_component.password, make_dir=True, root=str(config.files_directory))

    return (ResponseHeader.from_server(version=header_component.version, code=SuccessFlags.SUCCESSFUL_USER_CREATION, ended_connection=header_component.finish, config=config),
...
async def handle_login(header_component: BaseHeaderComponent,
    ...
    header: ResponseHeader = ResponseHeader.from_server(config=config, code=SuccessFlags.SUCCESSFUL_AUTHENTICATION, ended_connection=header_component.finish)
...
async def handle_password_change(header_component: BaseHeaderComponent,
    ...
    if not auth_component.password:
        raise InvalidAuthData(f'Missing new password in auth component')
    
    await user_manager.change_password(username=auth_component.identity, new_password=auth_component.password)
    ...
    header: ResponseHeader = ResponseHeader.from_server(config=config, version=header_component.version, code=SuccessFlags.SUCCESSFUL_PASSWORD_CHANGE)
...
async def handle_session_refresh(header_component: BaseHeaderComponent,
    ...
    assert auth_component.token and auth_component.refresh_digest

    # UserManager.refresh_session() implictly authenticates session
    new_digest, iteration = await user_manager.refresh_session(username=auth_component.identity,
                                                               token=auth_component.token,
                                                               digest=auth_component.refresh_digest)
    
    header: ResponseHeader = ResponseHeader.from_server(version=header_component.version,
                                                        code=SuccessFlags.SUCCESSFUL_SESSION_REFRESH,
