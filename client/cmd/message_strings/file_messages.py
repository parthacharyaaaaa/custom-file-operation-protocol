...
def succesful_file_deletion(remote_directory: str, remote_file: str,
                            revoked_info: Sequence[dict[str, Any]], deletion_time_string: Optional[str] = None,
                            code: Optional[SuccessFlags] = None) -> str:
    revocation_info_string: str = '\n---\n'.join(format_dict(i) for i in revoked_info) if revoked_info else "No revocation info available"
...
def failed_file_operation(remote_directory: str, remote_file: str,
                          operation: FileFlags, code: Optional[Union[ClientErrorFlags, ServerErrorFlags]] = None,
