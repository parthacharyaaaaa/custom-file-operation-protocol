...
def successful_ownership_trasnfer(remote_directory: str, remote_file: str, new_fpath: str,
                                  datetime_string: Optional[str] = None, code: Optional[SuccessFlags] = None) -> str:
    return f'''{(code or SuccessFlags.SUCCESSFUL_OWNERSHIP_TRANSFER).value}: Transferred ownership of file {remote_file} to {remote_directory}. You now have manager rights to this file.
    New Filepath: {new_fpath},
    transferred at: {datetime_string or "N/A"}'''

def successful_revoked_role(remote_directory: str, remote_file: str,
                            revoked_info: dict[str, str], code: Optional[SuccessFlags] = None) -> str:
...
def failed_permission_operation(remote_directory: str, remote_file: str, remote_user: Optional[str] = None,
                                code: Optional[Union[ClientErrorFlags, ServerErrorFlags]] = None, exc: Optional[Exception] = None) -> str:
