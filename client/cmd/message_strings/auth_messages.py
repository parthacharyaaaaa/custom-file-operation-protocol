...
def failed_auth_operation(operation: AuthFlags, code: ResponseCode) -> str:
    return '\n'.join([f'Code {code.value}: Failed auth operation',
...
def successful_authorization(remote_user: str, code: SuccessFlags = SuccessFlags.SUCCESSFUL_AUTHENTICATION) -> str:
    return f'Code {code.value}: Authorization successful, remote session created with identity {remote_user}'
...
def successful_reauthorization(remote_user: str, iteration: int, code: SuccessFlags = SuccessFlags.SUCCESSFUL_SESSION_REFRESH) -> str:
    return f'Code {code.value}: Refreshed remote session for user {remote_user}, session iterations: {iteration}'

def successful_logout(remote_user: str, code: SuccessFlags = SuccessFlags.SUCCESSFUL_SESSION_TERMINATION, **kwargs) -> str:
    return f'Code: {code.value}: Terminated remote session for {remote_user}.\n{format_dict(kwargs)}'
